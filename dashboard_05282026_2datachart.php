...
if(!$isRecommender && !$isApprover && !$isAdmin){
    ...
    $recentQuery .= " WHERE gs.area_id = '" . $conn->real_escape_string($area) . "'";
}

$recentQuery .= " AND gs.status != 'cancelled' GROUP BY gs.id ORDER BY gs.date_issued DESC LIMIT 5";
    ...
    if(!$isRecommender && !$isApprover && !$isAdmin){
        ...
        $sql = "
            SELECT 
                ...
                SUM(
                    CASE 
                        WHEN gs.status IN ('approved', 'printed')
                        THEN 1 
                        ELSE 0 
                    END
                ) AS approved_total,
        ...
        $sql = "
            ...
            INNER JOIN gas_slips gs
                ON gs.area_id = approver.area_id
...
$approvedMonthly = array_fill(0, 12, 0);
$createdMonthly  = array_fill(0, 12, 0);
...
if ($result) {
    while ($row = $result->fetch_assoc()) {
        ...
        $approvedMonthly[$monthIndex] = (int)$row['approved_total'];
        $createdMonthly[$monthIndex]  = (int)$row['created_total'];
    }
}

// $monthlyData = array_values($monthly);
$approvedData = $approvedMonthly;
$createdData  = $createdMonthly;
...
<div class="app-content">
    ...
    <main class="main-content">
        ...
        <div class="row g-4 mb-4">
                    ...
                    <div class="fw-semibold mb-0 px-3" style="padding-top: 10px; color: #64748b;">
                        <?php if ($isRecommender): ?>
                            ...
                            if (in_array($department_name, ['ASOD', 'ANOD'])) {
                                ...
                                echo $department_name . ' [' . $area_name . '] ';
                            }
                            ?>
            ...
            <!-- PIE CHART CARD (APPROVER ONLY) -->
            <?php if ($isApprover || $isAdmin): ?>
            <div class="col-md-6">
                <div class="card p-0 h-100">

                    <div class="fw-semibold mb-0 px-3" style="padding-top: 10px; color: #64748b;">
                         Department Distribution <!--(Approved – <?= htmlspecialchars($area) ?> Area) -->
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const canvas = document.getElementById('approvedLineChart');
    ...
    console.log('Approved:', <?= json_encode($approvedData) ?>);
    console.log('Created:', <?= json_encode($createdData) ?>);
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                ...
                {
                    label: 'Created Gas Slips',
                    data: <?= json_encode($createdData) ?>,
                    tension: 0.3,
                    fill: true,
                    borderColor: '#0d6efd',
