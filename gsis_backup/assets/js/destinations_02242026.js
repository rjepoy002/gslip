...
document.addEventListener('click', function (e) {
  ...
  document.getElementById('destOriginDisplay').value =
    USER_ORIGIN || '—';

  // ✅ STEP 1: set checkbox default FIRST
  applyRouteRules();

  // ✅ STEP 2: load destinations based on checkbox state
...
function applyRouteRules() {
  ...
  const category = activeDestinationRow.dataset.vehicleCategory || '';
...
/* =========================================================
   LOAD DESTINATIONS (BACKEND-DRIVEN)
========================================================= */
function loadDestinationsForOrigin(origin) {
  ...
  fetch(
    ...
    .then(data => {
      ...
      const hidden =
        activeDestinationRow.querySelector('.destinations-data');
      const saved = hidden?.value ? JSON.parse(hidden.value) : [];
      ...
      data.destinations
        ...
        .forEach(d => {
          ...
          const isChecked = saved.some(s => s.name === d.name);
