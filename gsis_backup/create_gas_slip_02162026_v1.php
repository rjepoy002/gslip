...
<script>
  ...
  if (form && saveBtn) {
    ...
    // SweetAlert leave warning (sidebar / browser nav)
    window.addEventListener('beforeunload', (e) => {
      if (!isDirty) return;
      e.preventDefault();
      e.returnValue = ''; // required for browser-level warning
    });

    // SweetAlert save protection
    form.addEventListener('submit', (e) => {
      if (saveBtn.dataset.confirmed === 'true') return;

      e.preventDefault();

      // ✅ BASIC VALIDATION (example: check if first row's purpose is filled)
      let hasError = false;
      ...
      document.querySelectorAll('#gasSlipGrid tbody tr').forEach(row => {
        ...
        if (!purpose || !requestedBy) {
          hasError = true;
          row.classList.add('table-danger'); // optional highlight
        } else {
      ...
      if (hasError) {
        Swal.fire({
          ...
          text: 'Purpose and Requested By are required for all rows.'
        });
        return;
      }
      // ✅ CONFIRMATION DIALOG
