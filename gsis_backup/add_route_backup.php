    ...
    if ($area && $origin && $destination && $distance_km) {
        if (isset($_POST['form_mode']) && $_POST['form_mode'] === 'edit' && isset($_POST['route_id']) && $_POST['route_id'] !== '') {
            ...
            if ($check->num_rows > 0) {
                ...
                $stmt = $conn->prepare("UPDATE routes SET origin=?, destination=?, distance_km=?, fuel_allocation=?, status=?, remarks=? WHERE id=?");
                $stmt->bind_param("ssddssi", $origin, $destination, $distance_km, $fuel_allocation, $status, $remarks, $id);
                if ($stmt->execute()) {
                    $success = true;
                } else {
