...
$stmtTemplates->bind_param("i", $userId);
$stmtTemplates->execute();

$templates = $stmtTemplates
    ->get_result()
        ...
        <div class="panel-container">
            ...
            <div>
              ...
              <?php
                ...
                if ($editMode && !empty($gasSlip['date_issued'])) {

                    $savedDate = date('Y-m-d', strtotime($gasSlip['date_issued']));

                    $displayDate = ($savedDate < $currentDate)
                        ? $currentDate
                        : $savedDate;
                ...
                echo date('F d, Y', strtotime($displayDate));
              ?>
            </div>
              ...
              <div class="d-flex align-items-center gap-2">
                ...
                <button type="button"
                  ...
                  <i class="fas fa-bookmark me-1"></i> Save Template
                </button>
            ...
            <table class="styled-table excel-table" id="gasSlipGrid">
                ...
                <td>
                    <?php
                        ...
                        if ($editMode && !empty($gasSlip['validity_until'])) {

                            $savedValidity = date('Y-m-d', strtotime($gasSlip['validity_until']));
                            $today         = date('Y-m-d');

                            // If expired/older than today → set to tomorrow
                            if ($savedValidity < $today) {
                                $validityValue = date('Y-m-d', strtotime('+1 day'));
                            } else {
                                $validityValue = $savedValidity;
                            }
                        }
                    ?>
                ...
                <td class="text-center fuel-cell">
                  ...
                  <input type="hidden"
                        name="fuel_requests[]"
                        class="fuel-data"
                        value='<?= htmlspecialchars(json_encode($fuelRows ?? []), ENT_QUOTES) ?>'>
...
if (!empty($_SESSION['area'])) {
    ...
    $stmtArea->bind_param("i", $_SESSION['area']);
    $stmtArea->execute();
