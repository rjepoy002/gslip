...
const USER_ORIGIN_NAME = window.APP?.USER_ORIGIN_NAME || '';
...
document.addEventListener('click', function (e) {
  ...
  // ignore anything inside modal body
  ...
  // ✅ FIXED
  const gasSlipId = cell.dataset.gasSlipId || null;
  ...
  // ✅ restore hidden input data for NEW rows / template rows
  if (!gasSlipId) {

    const json = activeDestinationRow
      .querySelector('.destinations-data')?.value || '[]';
    ...
    try {
      ...
      console.error('Invalid destination JSON', err);
    }
    ...
    setTimeout(() => {
      ...
      // ✅ IMPORTANT: sync hidden field immediately
  ...
  bootstrap.Modal
    .getOrCreateInstance(
      document.getElementById('destinationsModal')
    )
    .show();
...
function applyRouteRules() {
  ...
  // default only — user can override --> this works during create
  if (!category) {
    return;
  }

  useRoutes.checked = (category === '2-wheels');
}
...
function loadDestinationsForOrigin(origin) {
  ...
  fetch(
    ...
    .then(data => {
      ...
      // ✅ IMPORTANT:
      // sync hidden field automatically during preload
      if (activeDestinationRow) {
        ...
        const selected = Array.from(
          ...
          route_id: parseInt(cb.dataset.routeId) || null,
          name: cb.value,
...
document.getElementById('saveDestinationsBtn')
  ?.addEventListener('click', function () {
    ...
    // Update route values using the latest selected destinations
    updateSelectedDestinationsTable();
    
    // Recompute Diesel / Unleaded
    if (typeof recomputeFuelRequestsForRow === 'function') {
      recomputeFuelRequestsForRow(activeDestinationRow);
    }
