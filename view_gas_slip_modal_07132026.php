  ...
  <table class="table table-sm table-borderless mb-4 small text-muted">
    <tbody>
      ...
      <tr>
        ...
        <td>:</td>
        <td style="<?= $isExpired ? 'color:red;font-weight:bold;' : '' ?>">
          <?php echo date('M d, Y · h:i A', strtotime($data['validity_until'])); ?>
          <?= $isExpired ? ' (Expired)' : '' ?>
        </td>
      </tr>
      <tr>
...
<div class="modal-footer d-flex justify-content-between align-items-stretch border-top">
  ...
  <table class="table table-sm table-borderless mb-4 small text-muted">
      <tbody>
        ...
        <tr>
            <th class="text-muted w-25">Rejected By</th>
            <td>:</td>
            <td><?= htmlspecialchars(strtoupper($data['rejected_by'] ?? '')) ?></td>
        </tr>

        <tr>
            <th class="text-muted w-25">Rejected Date</th>
            <td>:</td>
            <td>
