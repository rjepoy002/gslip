...
<div class="modal-footer d-flex justify-content-between align-items-stretch border-top">
  ...
  <div class="d-flex align-items-end gap-2 ms-auto">
    ...
    <?php if (isset($role) && in_array($role, ['user', 'recommender']) && $data['status'] === 'pending' && $data['user_id'] == $_SESSION['user_id']): ?>
        <button
          type="button"
    ...
    <?php if (isset($role) && $role === 'recommender' && $data['status'] === 'pending'): ?>
        <button
          type="button"
    ...
    <?php elseif (isset($role) && $role === 'approver' && $data['status'] === 'recommended'): ?>
        <button
          type="button"
          class="btn btn-success"
          id="approveBtn"
