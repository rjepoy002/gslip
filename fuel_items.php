...
if (isset($_GET['delete'])) {
    ...
    $conn->query("UPDATE fuel_items SET status = 'inactive' WHERE id = $id");
}
    ...
    <main class="main-content">
        ...
        <div class="panel-container">
            ...
            <div class="page-header mb-4">
              <h1 class="mb-1">Fuel Items Management</h1>
              <p class="page-subtitle mb-1">
                  Manage fuel items with defined types, units, containers, and status to ensure accurate and consistent fuel requests.
              </p>
            </div>
            ...
            <!-- Recent Activity -->
            <table class="styled-table excel-table" id="FuelItemsTable">
                <thead>
                ...
                <?php if ($result->num_rows > 0): ?>
                  ...
                  <?php while ($row = $result->fetch_assoc()): ?>
                    ...
                    <tr class="FuelItems-row"
                        data-id="<?= $row['id'] ?>"
                        data-name="<?= htmlspecialchars($row['name']) ?>"
                        data-unit="<?= htmlspecialchars($row['unit']) ?>"
                        data-container="<?= htmlspecialchars($row['container']) ?>"
...
$(document).on('click', '.FuelItems-row', function () {
  ...
  const modal = new bootstrap.Modal(document.getElementById('addFuelItemsModal'));

  $('#fuel_item_id').val($(this).data('id'));
  $('#fuel_name').val($(this).data('name'));
  $('#unit').val($(this).data('unit'));
  $('#container').val($(this).data('container'));
  $('#status').val($(this).data('status'));
  $('#remarks').val($(this).data('remarks'));

  $('#formTitle').text('Edit Fuel Items');
...
function resetFuelItemsForm() {
    ...
    $('#formTitle').text('Add New Fuel Items');
