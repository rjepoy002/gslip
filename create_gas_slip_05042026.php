...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ==========================
    // SAVE GAS SLIP (existing)
    // ==========================
    if (isset($_POST['save_gasslip'])) {
    ...
    if (isset($_POST['btnTemplate'])) {
        ...
        // LOOP ROWS
        foreach ($_POST['vehicle_id'] as $i => $vehicle_id) {
            ...
            // ROW
            $stmt = $conn->prepare("
                INSERT INTO template_rows (template_id, vehicle_id, purpose, row_order)
                VALUES (?, ?, ?, ?)
            ");
            $stmt->bind_param("iisi", $template_id, $vehicle_id, $purpose, $row_order);
            $stmt->execute();

            $row_id = $stmt->insert_id;

            // ROUTES
            $dest_json = $_POST['destinations'][$i] ?? '[]';
            $destinations = json_decode($dest_json, true);

            foreach ($destinations as $index => $d) {
                $route_id = $d['route_id'] ?? 0;
                $route_order = $index + 1;

                $stmt = $conn->prepare("
                    INSERT INTO templates_routes (row_id, route_id, route_order)
                    VALUES (?, ?, ?)"
                );
                $stmt->bind_param("iii", $row_id, $route_id, $route_order);
                $stmt->execute();
            }

            // FUEL
            $fuel_json = $_POST['fuel_requests'][$i] ?? '[]';
            $fuels = json_decode($fuel_json, true);
            ...
            foreach ($fuels as $f) {
                ...
                $stmt = $conn->prepare("
                    INSERT INTO templates_fuel (row_id, fuel_item_id, quantity, container)
                    VALUES (?, ?, ?, ?)
                ");
                $stmt->bind_param("iids", $row_id, $fuel_id, $qty, $container);
                $stmt->execute();
            }
        }
...
<div class="app-content">
    ...
    <main class="main-content">
      <form id="gasSlipForm" method="post" action="save_gas_slip.php">
        ...
        <?php include 'includes/templates_modal.php'; ?>

        <input type="hidden" name="template_name" id="template_name_hidden">
        <input type="hidden" name="save_template" id="save_template_flag">
        ...
        <div class="panel-container">
            ...
            <div class="d-flex justify-content-between align-items-center text-muted small">
              ...
              <div>
                ...
                <?= $editMode && !empty($gasSlip['date_issued'])
                    ? date('F d, Y', strtotime($gasSlip['date_issued']))
                    : date('F d, Y') ?>
              </div>
              ...
              <button type="button"
                ...
                <i class="fas fa-bookmark me-1"></i> Save as Template
              </button>
            ...
            <?php if ($editMode): ?>
                <input type="hidden" name="gas_slip_id" value="<?= $gasSlip['id'] ?>">
            <?php endif; ?>
            ...
            <table class="styled-table excel-table" id="gasSlipGrid">
                ...
                <td>
                    <input type="date"
                          name="validity_until[]"
                          class="table-input"
                          value="<?= $editMode && isset($gasSlip['validity_until'])
                              ? htmlspecialchars($gasSlip['validity_until'])
                              : '' ?>">
                </td>
                <td>
                    <input type="text"
                          name="purpose[]"
                          class="excel-input"
                          value="<?= $editMode && isset($gasSlip['purpose'])
                ...
                <td>
                    <input type="text"
                          name="requested_by[]"
                          class="excel-input"
                          value="<?= $editMode && isset($gasSlip['requested_by'])
                              ? htmlspecialchars($gasSlip['requested_by'])
                              : '' ?>">
                </td>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('saveTemplateBtn').addEventListener('click', function () {
      ...
      // Sync to hidden input (important)
      document.getElementById('template_name_hidden').value = templateName;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (btn) {
      btn.addEventListener('click', function () {
        document.getElementById('template_name_hidden').value =
  ...
  document.getElementById('saveTemplateBtn').addEventListener('click', function () {
    ...
    const input = document.getElementById('template_name');
    ...
    document.getElementById('template_name_hidden').value = value;
    document.getElementById('save_template_flag').value = '1';

    // Optional: disable gas slip validation trigger
    document.getElementById('btnSave').dataset.confirmed = 'true';
